...
import detectIOS from 'lib/styles/detectIOS';
...
function WriterHead({ hide, children }: { hide: boolean; children: React.ReactNode }) {
  const screenHeight = (typeof window !== 'undefined' && window.screen.height) || 508;
...
export default class WriteMarkdownEditor extends React.Component<MarkdownEditorProps, MarkdownEditorState> {
  ...
  initialize = () => {
    ...
    this.codemirror.on('mousedown', (cm) => {
  ...
  handleOpenAddLink = () => {
    ...
    const calculatedTop = this.block.current.scrollTop + cursorPos.top + this.codemirror.defaultTextHeight() / 2 + 1;

    const isAtBottom = calculatedTop + 173 > this.block.current?.clientHeight;
    const pos = isAtBottom ? { top: null, bottom: 64 } : { top: calculatedTop, bottom: null };
    this.setState({
      addLink: {
        visible: true,
        ...pos,
  ...
  handleToolbarClickForApple = (mode: string) => {
    ...
    const textBelowCurrentLine = textAfter.slice(currentNewLineIndex, textAfter.length);
    ...
    const handlers: {
      ...
      heading1: () => {
        ...
        if (applied) {
          ...
          // onChangeMarkdown(`${textBefore}${replacedLine}${textBelowCurrentLine}`);
